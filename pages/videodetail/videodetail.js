...
Page({
  ...
  getContentData: function(isLoadmore) {
    ...
    app.requestWithSessionId({
      url: 'https://app.xiaogechui.cn/webservice/article/articles.ashx?action=QueryArtInfo&artid=' + that.data.artid, //50749
      data: {
        x: '',
        y: ''
      },
      ...
      success(res) {
        ...
        res.data.imgstyle = 'art-con-img'; //默认图片100%的宽度
        var videoHeight = systemInfo.windowWidth / scale;

        if (scale < 0.8 && 1 != imgfitw) {
          //将图片宽度设置成60%，样式2
          res.data.imgstyle = 'art-con-img-2';
          videoHeight = systemInfo.windowWidth * 0.6 / scale;
        }
  ...
  getPLData: function(isLoadmore) {
    ...
    var that = this;
    app.requestWithSessionId({
      url: 'https://app.xiaogechui.cn/webservice/article/comment.ashx?action=QueryCommentList&pageno=' + that.data.pagenum + '&stime=' + that.data.stime + '&artid=' + that.data.artid, //37625  50749
      data: {
        x: '',
        y: ''
      },
  ...
  onVideoClick: function() {
    ...
    if ("3" == this.data.artdata.atype) {
      //腾讯源视频
      vid = this.getParam('vid', this.data.artdata.attadress);
      ...
      //将点击视频进行播放
      var currPlayerContxt = TxvContext.getTxvContext(vid) //获取当前播放视频的上下文，可进行play，pause等操作
      ...
      var videoContext = wx.createVideoContext(this.data.artdata.artid)
      videoContext.play()
